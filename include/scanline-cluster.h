...
void saveIntensityAsPng(const std::vector<std::vector<SLRPointXYZIRL>> &laser_frame, const std::string &filename) {
    ...
    for (int i = 0; i < height; ++i) {
        for (int j = 0; j < width; ++j) {
            float normalized_intensity = (laser_frame[i][j].intensity - min_intensity) / (max_intensity - min_intensity);
...
ScanLineRun::ScanLineRun() :
    sensor_model_(16), th_run_(0.20), th_merge_(0.3), max_label_(1) {
