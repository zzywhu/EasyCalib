...
void FitPlaneSVD(Plane &single_plane) //????????????????��????????????????
{
    // 1??????????	// 2???????
...
public:
    ...
    void euclideanClusterSegmentation(
        ...
        float cluster_tolerance, // �������̶�
        int min_cluster_size,    // ��С�����С
...
void Calibration::addinitial3d(
    ...
    seg.setDistanceThreshold(0.01); // ������ֵ
    ...
    *lidar_line_cloud_3d_test = *lidar_line_cloud_3d;
    pcl::PointCloud<pcl::PointXYZI>::Ptr line_cloud(
        new pcl::PointCloud<pcl::PointXYZI>); // �����µĵ��ƶ������ڴ洢������
    line_cloud->height = 1;
    line_cloud->width = 0;
    // ��ȡ����������
    for (int i = 0; i < 3; ++i) {
...
void Calibration::euclideanClusterSegmentation(
    pcl::PointCloud<pcl::PointXYZI>::Ptr &input_cloud,
    std::vector<pcl::PointCloud<pcl::PointXYZRGB>::Ptr> &clusters,
    float cluster_tolerance = 0.01, // �������̶�
    int min_cluster_size = 10,      // ��С�����С
    int max_cluster_size = 25000)   // �������С
{
    // ����һ���յ�KD��������������
    ...
    ec.setClusterTolerance(cluster_tolerance); // ���þ�������̶ȣ���λ���ף�
    ec.setMinClusterSize(min_cluster_size);    // ������С�����С
    ec.setMaxClusterSize(max_cluster_size);    // �����������С
    ec.setSearchMethod(tree);                  // ����������ʽΪ KD ��
    ...
    // ��������������ÿ�����ౣ�浽 clusters �����У���Ϊÿ������䲻ͬ��ɫ
    for (const auto &cluster : cluster_indices) {
        pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud_cluster(
            new pcl::PointCloud<pcl::PointXYZRGB>);
        uint32_t cluster_color = getRandomColor(); // ��ȡ�����ɫ
        ...
        for (const auto &index : cluster.indices) {
            ...
            point.rgb = *reinterpret_cast<float *>(&cluster_color); // ���øõ����ɫ
...
pcl::PointCloud<pcl::PointXYZRGB>::Ptr processClustersWithBoundingBoxes(
    ...
    float sampling_density = 100.0f // ÿ��λ���ȵĲ�������
) {
    // �����ϳɽ���ĵ���
