...
int main(int argc, char **argv) {
    ...
    if (argc > 1) {
        calib_config_file = argv[1];
    }
    string configfile = calib_config_file;
    string ext_file;
    ...
    ext_file = parseStringFromYAML(configfile, "ext_file");
  ...
  pcd_path = "E:\\slam20_c_data\\lidar-camera\\lidar-camera-m\\data";*/
    ...
    vector<double> camera_matrix({1155.12539531, 0.0, 1035.8484391, 0.0,
                                  1154.54311767, 757.55707028, 0.0, 0.0, 1.0});
    vector<double> dist_coeffs(
        {-0.03154265, -0.01279613, 0.0138742, -0.00618993, 0});
    ...
    for (size_t i = 0; i < data_num; i++) {
        ...
        image_file = image_path + "/" + std::to_string(i + 1) + "." + ext_file;
        ...
        // pcl::io::loadPCDFile("check/plane_line_cloud.pcd",
        // *single_calib.plane_line_cloud_); pcl::io::loadPCDFile(EdgeFile,
        // *single_calib.plane_line_cloud_);
    ...
    calib_params[4] = T[1];
    calib_params[5] = T[2];
    // pcl::PointCloud<pcl::PointXYZRGB>::Ptr rgb_cloud(new
    // pcl::PointCloud<pcl::PointXYZRGB>); calibra.colorCloud(calib_params, 1,
    // calibra.image_, calibra.raw_lidar_cloud_, rgb_cloud);
    ...
    for (dis_threshold = 100; dis_threshold > 8; dis_threshold -= 1) {
        ...
        for (int cnt = 0; cnt < 2; cnt++) {
            ...
            opt_init_R = Eigen::AngleAxisd(euler_angle[0], Eigen::Vector3d::UnitZ()) * Eigen::AngleAxisd(euler_angle[1], Eigen::Vector3d::UnitY()) * Eigen::AngleAxisd(euler_angle[2], Eigen::Vector3d::UnitX());
    ...
    R = Eigen::AngleAxisd(calib_params[0], Eigen::Vector3d::UnitZ()) * Eigen::AngleAxisd(calib_params[1], Eigen::Vector3d::UnitY()) * Eigen::AngleAxisd(calib_params[2], Eigen::Vector3d::UnitX());
    ...
    // pcl::PointCloud<pcl::PointXYZRGB>::Ptr rgb_cloud_final(new
    // pcl::PointCloud<pcl::PointXYZRGB>); calibra.colorCloud(calib_params, 1,
    // calibra.image_,calibra.raw_lidar_cloud_, rgb_cloud_final);
...
//     if
//     (pcl::io::loadPCDFile<pcl::PointXYZ>("/home/linzp/newWS/calib_ws/src/livox_camera_calib/data/pcd/0.pcd",
//     *cloudA) == -1)
// 	{
...
//     visualizer.addPointCloud<pcl::PointXYZ> (cloudA, "source cloud
//     transformed");
